...
@Entity(
    tableName = "excursion_stops",
    foreignKeys = [
        ForeignKey(
            entity = ExcursionEntity::class,
            parentColumns = ["id"],
            childColumns = ["excursion_id"],
    ...
    indices = [
        Index(value = ["excursion_id"]),
        Index(value = ["country_iso2"]),
        Index(value = ["excursion_id", "sort_order"]),
    ],
)
data class ExcursionStopEntity(
    @PrimaryKey
    @ColumnInfo(name = "id")
    val id: String,
